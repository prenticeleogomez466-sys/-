...
test("XLSX_HEADERS:核心版10列齐全(2026-06-23 用户裁决:砍冗余·汇总一张主表)", () => {
  // 10列核心版(2026-06-23 用户裁决:盘口主推/组合触发三问/让球/比分/大小球/研判/注金;情报·盘口合理性·爆冷·决策辅助下沉专属sheet)
  ...
  assert.match(XLSX_HEADERS[3], /盘口主推/); // 主推方向+赔率+信心★
  assert.match(XLSX_HEADERS[4], /组合触发.*三问最可能/); // 胜平负/大小球/让球·命中%·★高确定性
  assert.match(XLSX_HEADERS[5], /让球.*主推vs市场/);
  assert.match(XLSX_HEADERS[6], /比分主推/);
  assert.match(XLSX_HEADERS[7], /大小球/);
  assert.match(XLSX_HEADERS[8], /研判/); // 看好谁·为什么·风险·一句话
  assert.match(XLSX_HEADERS[9], /注金/);
});
