...
test("handicapVerdictParts:与胜平负同向 → 注'与胜平负同向',无逻辑注", () => {
  ...
  assert.match(v.text, /让1球后胜 48%\(模型\) vs 43%\(市场\)/);
...
test("handicapVerdictParts:主胜但让球客胜(深盘)→ 不同向+'主胜但难净胜2球→让球客胜'逻辑注", () => {
  ...
  assert.match(v.note, /主胜但难净胜2球→让2球后负/);
  assert.match(v.text, /⚠️与胜平负不同向/);
  assert.match(v.text, /让2球后负 57%\(模型\) vs 48%\(市场\)/);
});
...
test("handicapVerdictParts:主胜但走盘(恰赢盘口球数)→ 走盘逻辑注", () => {
  ...
  assert.equal(v.sameDir, false);
  assert.match(v.note, /主胜但最可能恰好只赢1球→让球后打平/);
});
...
test("XLSX_HEADERS:核心版14列齐全(2026-06-25 用户令:对阵后加三套胜负平赔率独立列·欧洲/竞彩/让球各显主平客)", () => {
  // 14列核心版(2026-06-25:对阵后插欧洲/竞彩/让球三套1X2原始赔率独立列;其余列序顺延不变)
  assert.equal(XLSX_HEADERS.length, 14);
  assert.match(XLSX_HEADERS[0], /^#$/);
  assert.match(XLSX_HEADERS[1], /开赛/);
  assert.match(XLSX_HEADERS[2], /对阵.*情景/);
  assert.match(XLSX_HEADERS[3], /欧洲胜负平赔率.*主.*平.*客/); // 三套1X2列①欧洲(外盘均值🔶)
  assert.match(XLSX_HEADERS[4], /竞彩胜负平赔率.*主.*平.*客/); // ②竞彩不让球(✅500)
  assert.match(XLSX_HEADERS[5], /让球胜负平赔率.*主.*平.*客/); // ③竞彩让球(✅500)
  assert.match(XLSX_HEADERS[6], /盘口主推/); // 主推方向+赔率+信心★
  assert.match(XLSX_HEADERS[7], /组合触发.*条件.*触发规律.*三问/); // 2026-06-25 用户令:条件→触发规律→三问 揉进竞彩完整一格
  assert.match(XLSX_HEADERS[8], /让球.*主推vs市场/);
  assert.match(XLSX_HEADERS[9], /比分主推/);
  assert.match(XLSX_HEADERS[10], /半全场主推/);
  assert.match(XLSX_HEADERS[11], /大小球/);
  assert.match(XLSX_HEADERS[12], /研判/); // 看好谁·为什么·风险·一句话
  assert.match(XLSX_HEADERS[13], /注金/);
});
