...
// ── 2026-06-10 洞1:均势场+整数深让球线误报豁免(|线|≥1 阈值×5),真互换形态仍拦 ──
test("swapGuard: 均势场让-1(韩捷实测形态 4.11/1.17=3.5)不再误报", () => {
  const euro = { current: { home: 2.43, draw: 2.84, away: 2.74 } };   // ratio≈1.17
  const hc = { current: { home: 5.80, draw: 4.05, away: 1.41 } };     // ratio≈4.11
  assert.equal(swapGuardViolation(euro, hc, { line: -1 }), null);
});
test("swapGuard: 真互换形态(06-09事故,比值≈6.5)即使深线也拦", () => {
  const euro = { current: { home: 2.0, draw: 3.4, away: 3.0 } };      // ratio=1.7(实为让球)
  const hc = { current: { home: 1.17, draw: 6.5, away: 13 } };        // ratio≈11.1(实为1X2)
  assert.match(swapGuardViolation(euro, hc, { line: -2 }) ?? "", /互换残留/);
});
test("swapGuard: 让0/未知线保持×2原灵敏度", () => {
  const euro = { current: { home: 2.4, draw: 3.2, away: 2.9 } };      // ratio≈1.33
  const hc = { current: { home: 1.55, draw: 4.2, away: 5.6 } };       // ratio≈3.6 > 1.33*2
  assert.match(swapGuardViolation(euro, hc, { line: 0 }) ?? "", /互换残留/);
  assert.match(swapGuardViolation(euro, hc) ?? "", /互换残留/);
});
