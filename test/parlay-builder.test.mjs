...
test("buildParlayPlan:默认每档最多4注;极限高赔档存在且全部≥40倍", () => {
  ...
  const vol = plan.tiers.find((t) => t.tier === "🏆高配");
  assert.ok(vol, "比分6×比分6最高132倍,高配档(≥40)必须存在");
...
test("buildParlayPlan:💎性价比档=抽水最小;每注带价值/抽水;EV恒负(用未取整概率算,防伪正EV)", () => {
  const a = buildParlayLegs(mkPred(), JQS);
  const b = buildParlayLegs(mkPred({ fixture: { homeTeam: "丙队", awayTeam: "丁队", sequence: "5002" } }), JQS);
  const plan = buildParlayPlan([a, b]);
  const val = plan.tiers.find((t) => t.tier === "💎性价比");
  assert.ok(val, "💎性价比 档必须存在");
