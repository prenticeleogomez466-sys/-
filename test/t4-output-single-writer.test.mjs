...
test("三面同源:xlsx标题/手机页/英文页日期一致,双日期互不污染(#16)", () => {
  ...
  const outputs = dates.map((date) => {
    ...
    const mobile = renderMobileHtml({ date, rows: rows2, riskNote: "", intlN: 2, wcN: 0, auditFoot });
  ...
  for (const [i, o] of outputs.entries()) {
    ...
    // xlsx:标题行 + banner 行都是本日期;20列专业版列数不缩水
    assert.equal(o.sheets[0].rows[0][0], `⚡ 神选 · 竞彩完整覆盖 · ${o.date}`);
    assert.match(o.sheets[0].rows[1][0], new RegExp(o.date));
    assert.equal(XLSX_HEADERS.length, 20);
    assert.equal(o.sheets[0].rows[2].length, 20);
    assert.match(o.sheets[0].rows[2][19], /对抗证伪/);
...
test("resolveHtmlWriteTarget:固定页已是更新日期 → 改写日期副本不顶掉", () => {
  const newer = renderMobileHtml({ date: "2026-06-11", rows: rows2, riskNote: "", intlN: 2, wcN: 0, auditFoot: "" });
...
test("手机页/英文页:无当日审计 → 页脚不得出现历史写死背书", () => {
  const auditFoot = buildAuditFoot({ rows: rows2, advData: null });
  const mobile = renderMobileHtml({ date: "2026-06-10", rows: rows2, riskNote: "", intlN: 2, wcN: 0, auditFoot });
