...
// ---- 温度校准接进生产(闭环④:治过度自信,不改 argmax)----
import { applyTemperature, fitTemperature } from "../src/temperature-calibration.js";

test("applyTemperature T>1 软化概率(降最高项、升最低项)但不改 argmax", () => {
  const p = { home: 0.75, draw: 0.15, away: 0.10 };
  const soft = applyTemperature(p, 1.975);
  assert.ok(soft.home < p.home, "最高项被软化降低");
  assert.ok(soft.away > p.away, "最低项被抬高");
  const top = ["home", "draw", "away"].reduce((a, b) => (soft[b] > soft[a] ? b : a), "home");
  assert.equal(top, "home", "argmax 不变 → 命中率不受单调温度影响");
  const sum = soft.home + soft.draw + soft.away;
  assert.ok(Math.abs(sum - 1) < 1e-9, "仍是合法概率分布");
});

test("fitTemperature 对过度自信样本拟合出 T>1", () => {
  // 造一批"预测很自信但常错"的样本 → 应判过度自信、T>1
  const samples = [];
  for (let i = 0; i < 40; i++) {
    samples.push({ probabilities: { home: 0.8, draw: 0.12, away: 0.08 }, actual: i % 2 === 0 ? "3" : "0" });
  }
  const fit = fitTemperature(samples, { minSamples: 30 });
