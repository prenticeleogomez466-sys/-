...
export function buildLeagueExpertMixture(historicalMatches, opts = {}) {
  const gateK = opts.gateK ?? 60;
  const hp = fitHierarchicalPoisson(historicalMatches, opts);

  return {
    ok: hp.ok,
    global: hp.global,
    leagueCount: Object.keys(hp.leagues ?? {}).length,

    /** 单联赛的专家画像 + 门控权重(供解释/展示)。 */
    leagueExpert(competition) {
      const key = canonicalLeague(competition) ?? competition;
      const lp = hp.getLeagueParams(key);
      const n = lp.samples ?? 0;
      const w = n / (n + gateK);
      return {
        league: key,
        samples: n,
        weight: round(w),                 // 信本联赛的程度(0=全靠大模型,1=全靠本联赛)
        reliable: lp.reliable ?? false,
        baseRate: lp.baseRate,            // 已收缩的本联赛进球率
        homeAdvantage: lp.homeAdvantage,
        global: hp.global,
        explain: n >= 20
          ? `本联赛 ${n} 场,信本联赛 ${(w * 100).toFixed(0)}%`
          : `本联赛仅 ${n} 场,${(100 - w * 100).toFixed(0)}% 靠大模型先验兜底(防过拟合)`,
      };
    },

    /**
     * 一场比赛的最终 λ:联赛专家 λ × 类型专家强度系数。
     * teamCoef 可传球队 attack/defense(无则=1,纯联赛+类型层)。
     */
    expertLambda(competition, teamCoef = {}) {
      const key = canonicalLeague(competition) ?? competition;
      const g = hp.predictGoals(key, teamCoef);
      const prof = competitionProfile(competition);
      const le = this.leagueExpert(competition);
      return {
        league: key,
        leagueWeight: le.weight,
        lambdaHome: round(g.lambdaHome * prof.intensityMultiplier),
        lambdaAway: round(g.lambdaAway * prof.intensityMultiplier),
        expectedGoals: round((g.lambdaHome + g.lambdaAway) * prof.intensityMultiplier),
        typeProfile: { intensity: prof.intensityMultiplier, drawBoost: prof.drawProbBoost, upsetBoost: prof.upsetProbBoost },
        explain: le.explain,
      };
    },
  };
